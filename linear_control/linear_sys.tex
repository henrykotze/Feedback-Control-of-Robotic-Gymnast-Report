...
\begin{document}
	...
	\begin{tikzpicture}
	\node (wa) [wa]  {$\boldsymbol{\dot{x}}= \boldsymbol{A}\boldsymbol{x}+\boldsymbol{B}$};
	...
	\path [draw, ->] (C.south) |- node [above] {} 
	(feedback.east);
	
	\path [draw, ->,] (C.east) -- (Y.west);
	
	\path [draw, ->,] (feedback.west) -| ([xshift=-1cm]wa.west) -- (wa.west) {};
